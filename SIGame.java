...
public class SIGame {
	...
	private List<BigSwirlingInvader> bigSwirlingInvaders;
	private Tank tank;
	...
	public SIGame() {
		...
		bigSwirlingInvaders = new ArrayList<BigSwirlingInvader>();
		setUp();
	}
	...
	public List<BigSwirlingInvader> getInvaders() {
		return bigSwirlingInvaders;
	}
	...
	private void moveInvaders() {
		for (BigSwirlingInvader next : bigSwirlingInvaders) {
            next.move();
	...
	private void invade() {
		if (RND.nextInt(INVASION_PERIOD) < 1) {
			BigSwirlingInvader i = new BigSwirlingInvader(RND.nextInt(SIGame.WIDTH), 0);
			bigSwirlingInvaders.add(i);
		}
	}
	...
	private void checkCollisions() {
		...
		for (BigSwirlingInvader target : bigSwirlingInvaders) {
			if (checkInvaderHit(target, missilesToRemove)) {
				invadersToRemove.add(target);
			}
		}
		
		bigSwirlingInvaders.removeAll(invadersToRemove);
		missiles.removeAll(missilesToRemove);
	}
